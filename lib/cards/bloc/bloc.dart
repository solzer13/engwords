...
class CardsBloc extends Bloc<CardsBlocEvent, CardsBlocState> {
  ...
  CardsBloc({required this.settingsBloc, required this.wordsBloc})
  ...
  Future<void> _onStarted(
      CardsStarted event, Emitter<CardsBlocState> emit) async {
    try {
  ...
  FutureOr<void> _onWordsLoadedState(
      WordsLoadedState event, Emitter<CardsBlocState> emit) {
    try {
      if (_checkWords()) {
        card = CardsModel(
  ...
  FutureOr<void> _onCardsNextCard(
      CardsNextCard event, Emitter<CardsBlocState> emit) {
    try {
      card.word.repeat++;

      wordsBloc.add(WordsBlocEventEdit(card.word));
  ...
  FutureOr<void> _onPressVariant(
      CardsPressVariant event, Emitter<CardsBlocState> emit) async {
    try {
      if (!card.checkVariant(event.variant)) {
        card.word.repeat = 0;
      }
      emit(CardsLoaded(card: card));
    } catch (exception) {
  ...
  bool _checkWords() {
    if (wordsBloc.words.isNotEmpty) {
      if (wordsBloc.words
          .any((word) => word.repeat < settingsBloc.settings.countRepeatWord)) {
        return true;
      }
    }
